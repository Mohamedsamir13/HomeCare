  ...
  class AppointmentView extends GetView<AppointmentController> {
    ...
    Widget build(BuildContext context) {
      ...
      return Scaffold(
        appBar: AppBar(
            title: const AppText(
                text: 'Book Appointment', size: 20, fontWeight: FontWeight.bold)),
        body:Stack(
          children: [
        Positioned.fill(
        child: Image.asset(
    ...
    Widget _buildDaysGrid(BuildContext context) {
      return Obx(() {
        final firstDayOfMonth = DateTime(
            controller.selectedDate.value.year, controller.selectedDate.value.month, 1);
        final lastDayOfMonth = DateTime(
            controller.selectedDate.value.year, controller.selectedDate.value.month + 1, 0);
    ...
    Widget _buildTimeGrid(BuildContext context) {
      return Column(
        ...
        children: [
          ...
          Obx(() => GridView.builder( // تأكد من وجود Obx هنا
            shrinkWrap: true,
            gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
              crossAxisCount: 3,
